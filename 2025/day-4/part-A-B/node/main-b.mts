...
const crawl = () => {
  ...
  for (let i = 0; i < rows; i++)
  {
    for (let j = 0; j < columns; j++)
    {
      ...
      while (tempRow <= i + 1)
      {
        ...
        while (tempCol <= j + 1)
        {
          if (!data[tempRow]) // if row is undefined skip also we can't do row[-1][-1] as we are doing undefined[-1] we'll get an error
          {
            // jump to next row!
            tempCol = j + 2;
            continue;
  ...
  {
    data[overwrite[0]][overwrite[1]] = 'X'
  }
